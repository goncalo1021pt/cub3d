...
void	vp_scaled(t_session *instance)
{
	...
	{
		cam2d.offset.y = (y - cam2d.top_l.y) + (H_SCALE - (instance->player.y
					- cam2d.top_l.y));
		x = cam2d.top_l.x;
		while (x < cam2d.bot_r.x && instance->map.grid[y][x])
		{
			cam2d.offset.x = (x - cam2d.top_l.x) + (W_SCALE
					- (instance->player.x - cam2d.top_l.x));
			cam2d.color = get_pixel(&instance->mlx_img, cam2d.offset.x,
					cam2d.offset.y);
			if (instance->map.grid[y][x] == '1')
				pixel_put(instance, cam2d.offset.x, cam2d.offset.y, cam2d.color
					+ 0xf0f010);
...
// 		while (x < cam2d.bot_r.x && instance->map.grid[y][x])
// 		{
// 			cam2d.offset.x = (x - cam2d.top_l.x) + (MAP_SCALE * 3.5
					// - (instance->player.x - cam2d.top_l.x));
